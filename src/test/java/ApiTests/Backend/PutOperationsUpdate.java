...
public class PutOperationsUpdate {
    public boolean testPutOperationsUpdate(String siteUrl, TestUser user) throws IOException {
        long startTime;
        long elapsedTime;
        Operation originalOperation = new GetOperationsToRun(user).getAnyOperation(user, siteUrl);
        ...
        HttpURLConnection httpCon = MakeRequest.getConnection(siteUrl, user, "money/api/operations/update/", "PUT", "application/json", "application/json", true);
        ...
        Operation changedOperation = new GetOperationsToRun(user).getOperationByParameter("id", originalOperation.getId(), user, siteUrl);
        assertTrue("Check modified data saved correctly", modifiedOperation.equalsExceptUpdatedDate(changedOperation));

        // Содзаем URL
        httpCon = MakeRequest.getConnection(siteUrl, user, "money/api/operations/update/", "PUT", "application/json", "application/json", true);
        ...
        changedOperation = new GetOperationsToRun(user).getOperationByParameter("id", originalOperation.getId(), user, siteUrl);
